...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
            ...
            <tr>
                <td align="left" colspan="2">
                    <div class="checkBomTxt">BOM只有在WINDOWS下采用“记事本”存储为UTF-8时才会有。在UTF-8编码文件中BOM在文件头部，占用三个字节，以暗码的形式存在，用来标示该文件属于UTF-8编码，现在已经有很多软件识别BOM头，但是还有些不能识别BOM头，例如PHP就不能识别BOM头，这也是用记事本编辑UTF-8编码后执行就会出错的原因了。<br />
                        <span class="red">具体表现为：Cannot modify header information – headers already sent by ….</span>
                    </div>
                </td>
            </tr>
            <?php
        ...
        <div class="bottomToolbar">
            <a href="javascript:void(0);" onclick="form.submit();return false;" class="dataBtn">开始检查BOM</a>
            <form name="form" action="?" method="get">
                <span class="inputArea">
                    处理方式：
                    <input type="checkbox" name="autocl" id="autocl" value="1" checked="checked" />
                    <input type="hidden" name="action" id="action" value="check" /> &nbsp;自动清除&nbsp;&nbsp;
                    检查目录：
                    <input type="text" name="dir" id="dir" value="<?php echo ($dir = isset($dir) ? $dir : '/'); ?>" size="8" />
                    &nbsp;(默认为根目录)
                    &nbsp;&nbsp;<a href="check_bom.php">清空查询</a>&nbsp;&nbsp;&nbsp;
                </span>
            </form>
        </div>
        <?php
