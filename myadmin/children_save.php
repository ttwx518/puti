...
if ($action == 'add') {
    ...
    if (!isset($autodesc))
        $autodesc = '';
    if (!isset($autodescsize))
        $autodescsize = '';
    if (!isset($autopage))
        $autopage = '';
    if (!isset($autopagesize))
        $autopagesize = '';
    $auid = empty($auid)?0:intval($auid);
    ...
    (age, sex,address_prov,address_city,address_country,title2,slogan,hot,changjing,mengxiang,
    goodsid, siteid, classid, parentid, parentstr, mainid, mainpid, mainpstr, title, flag, 
    source, author, linkurl, keywords, description, content, picurl, picarr, orderid, hits, posttime, 
    checkinfo {$fieldname}) 
    VALUES ($age,$sex,'$address_prov','$address_city','$address_country','$title2','$slogan',$hot,'$changjing','$mengxiang',
     $goodsid, '$cfg_siteid', '$classid', '$parentid', '$parentstr', '$mainid', '$mainpid', '$mainpstr', '$title', '$flag',
     '$source', '$author', '$linkurl', '$keywords', '$description', '$content', '$picurl', '$picarr', '$orderid', '$hits', '$posttime',
...
//修改列表信息
    ...
    $sql = "UPDATE `$tbname` SET age=$age, sex=$sex, hot=$hot,changjing='$changjing', mengxiang='$mengxiang',
    address_prov='$address_prov',address_city='$address_city',address_country='$address_country',title2='$title2',slogan='$slogan', goodsid=$goodsid,
    siteid='$cfg_siteid', classid='$classid', parentid='$parentid', parentstr='$parentstr', mainid='$mainid', 
    mainpid='$mainpid', mainpstr='$mainpstr', title='$title', 
