    ...
    <body>
                ...
                while ($row = $dosql->GetArray()) {
                    ...
                    <tr align="left" class="dataTr">
                        ...
                        <td>
                            ...
                            <a href="javascript:void(0);" onclick="replyEvaluate(<?php echo $row['id']; ?>);" title="添加/更新(回复)"><?php echo ClearHtml($row['body']); ?></a>
                        </td>
                        <td id="reply_<?php echo $row['id']; ?>"><?php echo $row['reply']; ?></td>
                        <td><?php echo $row['molds']; ?></td>
                        <td><?php echo $row['aid']; ?></td>
                        <td class="number"><?php echo GetDateTime($row['time']); ?></td>
                        <td><?php echo $row['ip']; ?></td>
                        <td class="action endCol">
                            <span><a href="usercomment_save.php?id=<?php echo $row['id']; ?>&action=check&checkinfo=<?php echo $row['isshow']; ?>" title="点击进行审核与未审操作"><?php echo $checkinfo; ?></a></span> | 
                            <span><a href="usercomment_save.php?action=del2&id=<?php echo $row['id'] ?>" onclick="return ConfDel(0);">删除</a></span>
                        </td>
                    </tr>
                    <?php
                }
        ...
        <div class="bottomToolbar">
            <span class="selArea">
                ...
                <a href="javascript:DelAllNone('usercomment_save.php');" onclick="return ConfDelAll(0);">删除</a>
            </span>
        </div>
        ...
        <script>
            function replySubmit(id) {
                ...
                $.ajax({
                    url: 'ajax.php?action=replyEvaluate&id=' + id + '&replyContent=' + replyContent,
                    async: false,
                    type: 'post',
                    dataType: 'json',
            ...
            function replyEvaluate(id) {
                var replyContent = $('#reply_' + id).html();
                art.dialog({
                    id: id,
                    title: '回复评价',
                    content: '<textarea class="textarea" id="replyContent">' + replyContent + '</textarea>',
                    lock: true,
                    resize: false,
                    drag: false,
                    button: [
