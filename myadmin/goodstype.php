    ...
    <body>
                    ...
                    <div rel="rowpid_<?php echo GetTopID($row['parentstr']); ?>">
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
                            <tr align="left" class="dataTr">
                                ...
                                <td width="40%"><?php echo $classname; ?>
                                </td>
                                <td width="20%" align="center">
                                    <a href="goodstype_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>" class="leftArrow" title="提升排序"></a>
                                    <input type="text" name="orderid[]" id="orderid[]" class="inputls" value="<?php echo $row['orderid']; ?>" />
                                    <a href="goodstype_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>" class="rightArrow" title="下降排序"></a>
                                </td>
                                <td width="32%" class="action endCol">
                                    <span><a href="goodstype_add.php?id=<?php echo $row['id']; ?>">添加子类别</a></span> | 
                                    <span><a href="goodsattr.php?gid=<?php echo $row['id']; ?>">属性管理</a></span> | 
                                    <span><a href="goodstype_save.php?id=<?php echo $row['id']; ?>&action=check&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a></span> | 
                                    <span><a href="goodstype_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | 
                                    <span class="nb"><a href="goodstype_save.php?action=del&id=<?php echo $row['id'] ?>" onclick="return ConfDel(2);">删除</a></span>
                                </td>
                            </tr>
                        </table>
                    </div>
        ...
        <div class="bottomToolbar">
            <span class="selArea">
                ...
                <a href="javascript:DelAll('goodstype_save.php');" onclick="return ConfDelAll(2);">删除</a>
                <span>操作：</span>
                <a href="javascript:UpOrderID('goodstype_save.php');">排序</a> - 
                <a href="javascript:ShowAllRows();">展开</a> - 
                <a href="javascript:HideAllRows();">隐藏</a>
            </span>
            <a href="goodstype_add.php" class="dataBtn">添加商品类别</a>
        </div>
        <div class="page">
