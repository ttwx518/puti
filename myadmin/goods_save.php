...
if ($action == 'add') {
    ...
    //自动提取内容到摘要
    if ($autodesc == 'true') {
        if (empty($autodescsize) || !intval($autodescsize)) {
            $autodescsize = 200;
        }
    ...
    //用户组间接分销佣金处理
    $indirectCommission = serialize($indirectCommission);
...
//修改商品信息
    ...
    $sql = "UPDATE `$tbname` SET salenum=$salenum,
           classid='$classid', parentid='$parentid', parentstr='$parentstr', typeid='$typeid', typepid='$typepid', 
           typepstr='$typepstr', brandid='$brandid', brandpid='$brandpid', brandpstr='$brandpstr', title='$title', 
           colorval='$colorval', boldval='$boldval', flag='$flag', attrstr='$attrstr', weight='$weight', 
           goodsid='$goodsid', payfreight='$payfreight', freight='$freight', salesprice='$salesprice', 
