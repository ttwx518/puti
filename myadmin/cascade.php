    ...
    <body>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
                ...
                <tr align="left" class="dataTr">
                    <td height="36" class="firstCol">
                        <input type="checkbox" name="checkid[]" id="checkid[]" value="<?php echo $row['id']; ?>,|,<?php echo $row['groupsign']; ?>">
                    </td>
                    <td><?php echo $row['id']; ?></td>
                    <td>
                        <input type="text" name="groupname[]" id="groupname[]" class="inputd" value="<?php echo $row['groupname']; ?>" />
                        <input type="hidden" name="id[]" id="id[]" value="<?php echo $row['id']; ?>" />
                    </td>
                    <td><input type="text" name="groupsign[]" id="groupsign[]" class="inputd" value="<?php echo $row['groupsign']; ?>" /></td>
                    <td align="center">
                        <a href="cascade_save.php?id=<?php echo $row['id']; ?>&amp;orderid=<?php echo $row['orderid']; ?>&amp;action=up" class="leftArrow" title="提升排序"></a>
                        <input name="orderid[]" type="text" id="orderid[]" class="inputls" value="<?php echo $row['orderid']; ?>" />
                        <a href="cascade_save.php?id=<?php echo $row['id']; ?>&amp;orderid=<?php echo $row['orderid']; ?>&amp;action=down" class="rightArrow" title="下降排序"></a>
                    </td>
                    <td class="action endCol">
                        <span><a href="cascadedata.php?sign=<?php echo $row['groupsign'] ?>">查看</a></span> | 
                        <span class="nb"><a href="cascade_save.php?action=delclass&amp;sign=<?php echo $row['groupsign'] ?>&amp;id=<?php echo $row['id'] ?>" onclick="return ConfDel(2);">删除</a></span>
                    </td>
                </tr>
                <?php
                    }
        ...
        <div class="bottomToolbar">
            <span class="selArea">
                ...
                <a href="javascript:SubUrlParam('cascade_save.php?action=delallclass');" onclick="return ConfDelAll(2);">删除</a>
                <span>操作：</span>
                <a href="javascript:UpOrderID('cascade_save.php');">更新排序</a>
            </span>
            <a href="javascript:void(0);" onclick="form.submit();return false;" class="dataBtn">更新全部</a>
        </div>
        <div class="page">
