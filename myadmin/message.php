...
<body>
<div class="topToolbar"> <span class="title">爱心留言管理</span> <a href="javascript:location.reload();" class="reload">刷新</a></div>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		while($row = $dosql->GetArray())
		{
			switch($row['checkinfo'])
			{
				case '1':
					$checkinfo = '已审核';
					$checkurl="message_save.php?id=".$row['id']."&action=update&checkinfo=0";
					break;  
				case '':
					$checkinfo = '未审核';
					$checkurl="message_save.php?id=".$row['id']."&action=update&checkinfo=1";
					break;
				default:
					$checkinfo = '没有获取到参数';
			}
		...
		<?php $member = $dosql->GetOne("select * from `#@__member` where id={$row['uid']}")?>
			...
			<td class="action endCol"><span><a href="<?php echo $checkurl?>" title="点击进行审核与未审操作"><?php echo $checkinfo; ?></a></span> | <span class="nb"><a href="message_save.php?action=del2&id=<?php echo $row['id']; ?>" onclick="return ConfDel(0);">删除</a></span></td>
		</tr>
		<?php
		}
		?>
...
?>
<div class="bottomToolbar"> <span class="selArea"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:DelAllNone('message_save.php');" onclick="return ConfDelAll(0);">删除</a>-
          </span></div>
