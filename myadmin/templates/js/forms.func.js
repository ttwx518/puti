...
//审核选中提示
function ConfreviewAll(i){
    var tips = Array();
    tips[1] = "确定要将选中的信息标识为审核失败吗？";
    tips[2] = "确定要将选中的信息标识为审核成功吗？";
    if ($("input[type='checkbox'][name!='checkid'][name^='checkid']:checked").size() > 0){
        if (confirm(tips[i]))
            return true;
        else
...
//删除所有(不包含子分类)
function UpAllNone(url, status){
    $("#form").attr("action", url + "?action=upall2&status="+status).submit();
}
