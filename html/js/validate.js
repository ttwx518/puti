...
var member = {
    ...
    checkImprove: function() {
        ...
        if(!username){
            ShowMessage('请填写用户名(手机号)');
            $username.focus();
            return false;
        }
        if(!validateRules.isMobile(username)){
            ShowMessage('请填写正确的用户名(手机号)');
            $username.focus();
            return false;
        }
        ...
        if(!validateRules.betweenLength(pwd, 6, 20)){
            ShowMessage('请填写6~20位长度的密码');
            $pwd.focus();
            return false;
        }
        if(!validateRules.isPwd(pwd)){
            ShowMessage('密码只能由字母、数字、下划线组成');
            $pwd.focus();
            return false;
        }
        var $repwd = $('#repwd'),
    ...
    checkTx: function(){
        ...
        if(!validateRules.isIntege(amount)){
            ShowMessage('提现金额必须为1的整数倍');
            $amount.focus();
            return false;
        }
        if(amount < 200){
            ShowMessage('提现金额最少为200元');
            $amount.focus();
            return false;
        }
        if(amount > maxAmount){
        	ShowMessage('提现金额必须小于可提现金额');
            $amount.focus();
            return false;
        }
    }
